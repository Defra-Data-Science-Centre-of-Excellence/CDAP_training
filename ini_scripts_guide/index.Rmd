...
Init scripts are (usually small) shell scripts that run during the start-up of each cluster node before the Spark driver or worker JVM starts. We primarily use these scripts to pre-install Python and R libraries. We can also use them to add certificates that we might need to access specific resources.  
...
Initial Assumptions:  
1.	The user has access to Databricks.  
2.	The user can run a command in a notebook.  
3.	The user can create a PC compute cluster.  
4.	The user can create a file in their respective workspace.  
...
In the Workspace Explorer right click on the filename and select “Copy > Path”.  
Path is now copied to clipboard. It should like something like this:   /Workspace/Users/firstname.lastname\@defra.gov.uk/my_init_script.sh
