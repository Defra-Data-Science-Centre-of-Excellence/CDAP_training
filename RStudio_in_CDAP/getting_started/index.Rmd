...
Another way to see the packages that are installed is to go to the bottom right pane in RStudio and click on `Packages`, which will show both the `User Library` with packages you have installed yourself, and `System Library` with all pre-installed pacvkages.  

When the runtime of Databricks is updated on CDAP, so will the list and versions of R packages.   

You can also install further packages yourself in the usual ways, by typing the code below into the console and pressing `Enter`:
