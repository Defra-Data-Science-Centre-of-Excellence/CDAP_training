...
This guide is based on Jenny Bryan’s book, Happy Git with R. I have added the necessary steps from the book, with some CDAP-specific amendments. The book is an excellent, freely available resource and offers lot of advice and troubleshooting: [**Let’s Git started | Happy Git and GitHub for the useR**](https://happygitwithr.com)  
...
![](images/TerminalOptions.mp4){#id .class width=75%}
...
### Using the RStudio terminal on CDAP  
...
```{r, echo = FALSE, out.width = '75%', fig.alt = "Outputs from running the code, highlighting the key part on whether the PAT has been discovered"}
knitr::include_graphics("images/UseThis.png", dpi = NA)
```
...
![](images/CreatingRepo.mp4){#id .class width=75%}
...
![](images/OpeningRepo.mp4){#id .class width=75%}
...
```{r, echo = FALSE, out.width = '50%', fig.alt = "Username field when adding GitHub repo"}
knitr::include_graphics("images/GitHubUsername.png", dpi = NA)
```
...
- Add the URL of your RStudio on CDAP, your GitHub username, and the PAT for the password.  
...
```{r, echo = FALSE, out.width = '50%', fig.alt = "Setup of RStudio shown once project has been added from GitHub including added Git tab and project files"}
knitr::include_graphics("images/GitHubProject.png", dpi = NA)
```

#### Editing a file and adding changes to GitHub

![](images/EditingRepoInRStudio.mp4){#id .class width=75%}
...
We will now do the following process. We will commit the changes – this means we are communicating that we have changed something, and what exactly we have changed. We will then pull from GitHub – that means we are getting the latest version of the code from GitHub (relevant if you work on the code with others, or if you work on the project from more than one source, for example CDAP and your local machine). We will then push our changes to GitHub, so our latest changes will be added to GitHub.  
...
```{r, echo = FALSE, out.width = '50%', fig.alt = "View of repo on GitHub including text that has been added through RStudio"}
knitr::include_graphics("images/TextInRepo.jpeg", dpi = NA)
```
